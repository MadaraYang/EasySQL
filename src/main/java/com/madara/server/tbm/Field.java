...
import com.madara.server.im.BPlusTree;
import com.madara.server.tbm.impl.TableManagerImpl;
import com.madara.server.tm.impl.TransactionManagerImpl;
import com.madara.sqlparser.statement.SingleExpression;
import com.madara.utils.ParseStringRes;
...
//    B树根节点位置
    ...
    public static Field loadField(Table tb, long uid) {
        byte[] raw = null;
        try {
            raw = ((TableManagerImpl)tb.tbm).vm.read(DEFAULT_XID, uid);
        } catch (Exception e) {
            Exit.systemExit(e);
        }
        assert raw != null;
    ...
    public ParseValueRes parserValue(byte[] raw) {
        ...
        switch(fieldType) {
            case "int32":
                ...
                break;
            case "int64":
                res.v = Parser.byte2Long(Arrays.copyOf(raw, 8));
                res.shift = 8;
                break;
            case "string":
